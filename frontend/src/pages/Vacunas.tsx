...
function Vacunas() {
  ...
  const registrarVacunacion = async (e: FormEvent) => {
    ...
    try {
      await createRegistroVacunacion({
      ...
      setMensaje('Vacunación registrada correctamente')
      setIdAnimal('')
      setIdVacuna('')
      setFechaAplicacion('')
      setProximaFecha('')
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-gray-800 mb-2">Vacunas y salud</h1>
      <p className="text-gray-500 mb-6">Administra vacunas y registros sanitarios.</p>

      {error && <div className="bg-red-100 text-red-700 p-4 rounded-lg mb-4">{error}</div>}
      {mensaje && <div className="bg-green-100 text-green-700 p-4 rounded-lg mb-4">{mensaje}</div>}

      <section className="bg-white rounded-xl shadow p-6 mb-8">
        <h2 className="text-xl font-semibold mb-4">{editandoId ? 'Editar vacuna' : 'Crear vacuna'}</h2>
        ...
        <form onSubmit={guardarVacuna} className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <input type="number" className="border rounded-lg px-4 py-2" placeholder="Frecuencia días" value={frecuenciaDias} onChange={(e) => setFrecuenciaDias(e.target.value)} />
          ...
          <div className="flex gap-2">
            ...
            {editandoId && (
              <button type="button" onClick={limpiarVacuna} className="bg-gray-500 text-white px-4 py-2 rounded-lg">
                Cancelar
              </button>
            )}
          </div>
      ...
      <section className="bg-white rounded-xl shadow p-6 mb-8">
        <h2 className="text-xl font-semibold mb-4">Registrar vacunación</h2>
        ...
        <form onSubmit={registrarVacunacion} className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <select className="border rounded-lg px-4 py-2" value={idVacuna} onChange={(e) => setIdVacuna(e.target.value)}>
          ...
          <input type="date" className="border rounded-lg px-4 py-2" value={fechaAplicacion} onChange={(e) => setFechaAplicacion(e.target.value)} />
          <input type="date" className="border rounded-lg px-4 py-2" value={proximaFecha} onChange={(e) => setProximaFecha(e.target.value)} />
          <input className="border rounded-lg px-4 py-2" placeholder="Veterinario" value={veterinario} onChange={(e) => setVeterinario(e.target.value)} />
      ...
      <section className="bg-white rounded-xl shadow p-6 mb-8">
        <h2 className="text-xl font-semibold mb-4">Catálogo de vacunas</h2>
              ...
              {vacunas.map((vacuna) => (
                <tr key={vacuna.id_vacuna} className="border-b">
                  ...
                  <td className="flex gap-2 py-3">
                    <button onClick={() => editarVacuna(vacuna)} className="bg-blue-600 text-white px-3 py-1 rounded-lg text-sm">Editar</button>
                    <button onClick={() => eliminarVacuna(vacuna.id_vacuna)} className="bg-red-600 text-white px-3 py-1 rounded-lg text-sm">Eliminar</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <section className="bg-white rounded-xl shadow p-6">
        <h2 className="text-xl font-semibold mb-4">Historial de vacunación</h2>
              ...
              <tbody>
                {registros.map((registro) => (
                  <tr key={registro.id_registro} className="border-b">
                    ...
                    <td>
                      <button onClick={() => eliminarRegistro(registro.id_registro)} className="bg-red-600 text-white px-3 py-1 rounded-lg text-sm">
                        Eliminar
                      </button>
                    </td>
                  </tr>
